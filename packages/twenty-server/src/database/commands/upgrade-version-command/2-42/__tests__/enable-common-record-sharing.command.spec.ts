...
describe('Common sharing upgrade', () => {
  ...
  it.each([true, false])('only skips missing metadata when storage is unprovisioned and empty (%s)', async (isEmpty) => {
    ...
    else await expect(result).rejects.toThrow('Conversation metadata must be provisioned');
    expect(migrations.validateBuildAndRunLegacyWorkspaceMigration).not.toHaveBeenCalled();
  });
